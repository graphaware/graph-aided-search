...
package com.graphaware.integration.es.reco.demo.engine;
...
public class RecruitPeopleBySkills extends SingleScoreRecommendationEngine<Node, Node> {
    ...
    protected Map<Node, PartialScore> doRecommendSingle(Node company, Context<Node, Node> context) {
        ...
        for (Relationship looksFor : company.getRelationships(DynamicRelationshipType.withName("LOOKS_FOR_SKILL"), Direction.OUTGOING)) {
            Node skill = looksFor.getEndNode();
            for (Relationship hasSkill : skill.getRelationships(DynamicRelationshipType.withName("HAS_SKILL"), Direction.INCOMING)) {
